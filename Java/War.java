...
(u, v)
c = 1, setFriends
 - conflict: u >><< v --> areEnemies(u, v) == true
 - ok:
    - find up, vp
    - union(up, vp)
     +  parent[up] = vp
    - union(enemy[up], enemy[vp])
     +  eup = enemy[up]
     +  evp = enemy[vp]
     +  parent[eup] = evp
     +  enemy[vp] = evp
     +  enemy[evp] = vp
c = 2, setEnemies
 - conflict: u ~~~~ v --> areFriends(u, v) == true
 - ok:
   - find up, vp
   - eup = enemy[up]
   - evp = enemy[vp]
...
enemyArr: enemyArr[u] là phần tử đại diện cho tập enemy của tập u (enemyArr[up] - vp)
*/
/**
...
=>> not in the same version -> enemies
