...
class Brackets {
    ...
    public static void main(String[] args) {
        ...
        while(sc.hasNext()) {
            ...
            for (int i=0; i < m; i++) {
                int c = sc.nextInt();
                if (c == 0) { // check
                    if (n % 2 == 0 && segmentTree[0].numCloseBrackets == 0 && segmentTree[0].numOpenBrackets == 0) {
    ...
    private static void buildTree(char[] arr, Node[] segmentTree, int left, int right, int idx) {
        ...
        int newMatch = Math.min(segmentTree[2 * idx + 1].numOpenBrackets, segmentTree[2 * idx + 2].numCloseBrackets);
        int numOpenBrackets = segmentTree[2 * idx + 2].numOpenBrackets + segmentTree[2 * idx + 1].numOpenBrackets - newMatch;
        int numCloseBrackets = segmentTree[2 * idx + 2].numCloseBrackets + segmentTree[2 * idx + 1].numCloseBrackets - newMatch;
        segmentTree[idx] = new Node(numOpenBrackets, numCloseBrackets);
    }
    private static void updateQuery(char[] arr, Node[] segmentTree, int left, int right, int idx, int pos) {
        ...
        int newMatch = Math.min(segmentTree[2 * idx + 1].numOpenBrackets, segmentTree[2 * idx + 2].numCloseBrackets);
        segmentTree[idx].numOpenBrackets = segmentTree[2 * idx + 2].numOpenBrackets + segmentTree[2 * idx + 1].numOpenBrackets - newMatch;
        segmentTree[idx].numCloseBrackets = segmentTree[2 * idx + 2].numCloseBrackets + segmentTree[2 * idx + 1].numCloseBrackets - newMatch;
    }
}
class Node {
