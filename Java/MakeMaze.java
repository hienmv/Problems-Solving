/**
 * https://www.spoj.com/problems/MAKEMAZE/ #bfs set all the valid point in the border. count > 2,
 * check each couple of them that there are any road between them
 */
...
class MakeMaze {
  ...
  static boolean checkPoint(Point a, Point b, char[][] graph) {
    ...
    while (!queue.isEmpty()) {
      ...
      /**
       * duyet ma tran, it khi chuyen ve dang tong quat tao 2 mang chua gia tri thay doi cua x, y tx
       * - 1 ty + 0 tx + 1 ty + 0 tx + 0 ty - 1 tx + 0 ty + 1
       *
       * <p>dx = [-1, 1, 0, 0] dy = [ 0, 0, -1, 1] for (int dir = 0; dir < 4; dir++) { if
       * (validPoint(t.x + dx[dir], t.y + dy[dir], visitedGraph, graph)) { // visit } }
       */
