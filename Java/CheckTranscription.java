...
kokokokolin -> number of characters is fixed.
 -> choose prefix for '0' (fixed) ... substring for '1'
 -> number of choosen ways depends on a somehow formular.
...
for l0 : 1 -> len_t
    if ((len_t - c0 * r0) % c1 != 0) continue;
    l1 = (len_t - c0 * r0) / c1
    
    => select l0 -> select a proper l1.
    => optimize the comprasion of two substring -> hashing.
 */
