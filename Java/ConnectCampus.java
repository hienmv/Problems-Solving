...
public class ConnectCampus {
	public static void inputConnectedCampusMap( HashMap<Integer, HashSet<Integer>> connectedCampusMap, int idx1, int idx2) {
		HashSet<Integer> set;
	...
	public static void Prim(ArrayList<ArrayList<Node>> graph, HashMap<Integer, HashSet<Integer>> connectedCampusMap, int src, int[] distArr) {
		int sz = graph.size();
	...
	public static void main(String[] args) {
		...
		while(sc.hasNextInt()) {
			...
			for (int i=1; i < sz; i++) {
				...
				for (int j=1; j < sz; j++) {
					if (i==j) continue;
					distance = (int)Math.pow(arrCampus[i].x - arrCampus[j].x, 2) + (int)Math.pow(arrCampus[i].y - arrCampus[j].y, 2);
