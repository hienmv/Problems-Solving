/**
 * https://codeforces.com/problemset/problem/264/B #dynamic-programming refactor => get all
 * dividable prime of a number -> map:[prime:max divided number]
 *
 * <p>number: => primes for prime in primes: => count[number] = max(count[number],
 * count[map.get(prime)] + 1) update map[prime] = number result = max(result, count[number])
 */
