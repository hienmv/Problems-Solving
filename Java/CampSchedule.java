...
class CampSchedule {
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        StringBuilder res = new StringBuilder(t);
        if (!suffix.isEmpty()) {
            while(source.num0 >= need.num0 && source.num1 >= need.num1
                && need.num0 > 0 && need.num1 > 0) {
                res.append(suffix);
                source.num0 -= need.num0;
                source.num1 -= need.num1;
            }
        }
        while(source.num0 >= dest.num0 && dest.num0 > 0 
        && source.num1 >= dest.num1 && dest.num1 > 0) {
            res.append(t);
            source.num0 -= dest.num0;
            source.num1 -= dest.num1;
        }
    ...
    public static String maxSuffix(int idx, long[] mul, int[] arr0,
    int[] arr1, String t, long hashCode, Number source, Number need)
    {
        ...
        if ((hashArr[len] == ((hashArr[n] - (hashArr[n - len] * mul[len]) % MOD) + MOD) % MOD)
            && source.num0 >= need0 && source.num1 >= need1) {
            need.num0 = need0;
            need.num1 = need1;
