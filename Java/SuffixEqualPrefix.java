 ...
 * <p>hash(l, r)
 *
 * <p>hash[r] = hash(0, r) = a[0] * k^(n-1) + a[1] * k^(n-2) + ... + a[r] * k^(x) hash[l - 1] =
 * hash(0, l-1) = a[0] * k^(n-1) + a[1] * k*(n-2) + ... + a[l-1] * k^(y)
 *
 * <p>12345 123 * k^p 210
 *
 * <p>hash[0] = 0 for i: 1 ... n: hash[i] = (hash[i - 1] * BASE + a[i] - 'a') % MOD
 *
 * <p>pw[0] = 1 for i: 1 ... n: pw[i] = (pw[i - 1] * BASE) % MOD
 *
 * <p>hash(l, r) = (hash[r] - (hash[l - 1] * pw[r - l + 1]) % MOD + MOD) % MOD
 *
 * <p>pre[0..i] = suf[j..n-1] hash(1, i+1) ... hash(j + 1, n)
 *
 * <p>abcabc abc
 */
